...
export default function Cart() {
  let{cart,UpdateProductCountToCart,deleteProductToCart}=useContext(CartContext)
      ...
      {cart.data.products.map((item,index)=><tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
        ...
        <td className="px-6 py-4">
          <div className="flex items-center">
            <button  onClick={item.count>item.count>1 ? ()=>UpdateProductCountToCart(item.product.id ,item.count-1):()=>deleteProductToCart(item.product.id)} className="inline-flex items-center justify-center p-1 me-3 text-sm font-medium h-6 w-6 text-gray-500 bg-white border border-gray-300 rounded-full focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700" type="button">
        ...
        <td className="px-6 py-4">
          <button onClick={()=>deleteProductToCart(item.product.id)} className="bg-transparent hover:bg-transparent font-medium text-red-600 dark:text-red-500 hover:underline">Remove</button>
        </td>
      </tr>
 )}
...
<div className="flex justify-between pt-3">
  <h3 className='text-3xl'>Total Cart Price: {cart.data.totalCartPrice}</h3>
